...
from haim_drl.algo.gail.exp_saver import Experiment

from haim_drl.algo.gail.mlp import Policy, Value
...
expert_data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), '/home/zilin/code/HAIM-DRL/haim_drl/utils/human_traj_100_0.06.json' )
...
class Learner:
    ...
    def _collect_samples(self):
        ...
        total_episode_disturbance = []  # Count the total number of disturbances

        episode_disturbance_cost_mean = [0 for _ in range(self.env_num)]    # Initialize episode_disturbance_cost_mean
        total_episode_disturbance_cost = 0  # Initialize total_episode_disturbance_cost
        ...
        for i in range(self.buffer_length):
            ...
            total_episode_disturbance += [float(info[idx]["disturbance"]) for idx in range(self.env_num)]
            ...
            episode_disturbance_cost_mean = [episode_disturbance_cost_mean[i] + info[i]["disturbance_cost"] for i in range(self.env_num)]   # Calculate episode_disturbance_cost_mean
            ...
            for idx, done in enumerate(dones):
                if done:
                    ...
                    total_episode_disturbance_cost += episode_disturbance_cost_mean[idx]    # Calculate total_episode_disturbance_cost

                    episode_reward_mean[idx] = 0
                    episode_cost_mean[idx] = 0

                    episode_disturbance_cost_mean[idx] = 0  # Reset episode_disturbance_cost_mean
        ...
        return {"episode_reward_mean": total_episode_reward / done_num,
                ...
                "episode_disturbance_cost_mean": total_episode_disturbance_cost / done_num, # Record episode_disturbance_cost_mean

                "episode_disturbance_mean": np.mean(total_episode_disturbance),  # Record episode_disturbance_count_mean
    ...
    def train(self, is_train):
        ...
        metrics["episode_disturbance_cost"] = sample_result["episode_disturbance_cost_mean"]
        metrics["episode_disturbance"] = sample_result["episode_disturbance_mean"]
    ...
    def evaluation(self, evaluation_episode_num=30):
        ...
        episode_cost = 0
        episode_disturbance_cost = 0    #  Initialize total episode_disturbance_cost
        ...
        while episode_num < evaluation_episode_num:
            ...
            # state = torch.tensor(np.array(state)).to(self.cfg.device).float()
            ...
            disturbance.append(float(info["disturbance"]))

            state = next_state
            episode_reward += r
            episode_cost += info["native_cost"]

            episode_disturbance_cost += info["disturbance_cost"]    # Calculate total_episode_disturbance_cost
        ...
        res = dict(
            ...
            mean_episode_disturbance_cost=episode_disturbance_cost / evaluation_episode_num,    # Record mean_episode_disturbance_cost

            mean_velocity=np.mean(velocity),

            mean_disturbance=np.mean(disturbance),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epoch', default=1000000)
