...
class HAIM_DRLCallbacks(DrivingCallbacks):
    def on_episode_start(
            self, *, worker: RolloutWorker, base_env: BaseEnv, policies: Dict[str, Policy], episode: MultiAgentEpisode,
            env_index: int, **kwargs
    ):
        ...
        episode.user_data["disturbance"] = 0    # Number of traffic disturbances
        episode.user_data["total_disturbance_cost"] = 0
    ...
    def on_episode_step(
        ...
        if info is not None:
            ...
            episode.user_data["disturbance"] += 1 if info["disturbance"] else 0
            episode.user_data["total_disturbance_cost"] += info["disturbance_cost"]
    ...
    def on_episode_end(
        ...
        episode.custom_metrics["disturbance_rate"] = float(episode.user_data["disturbance"] / episode.length)
        episode.custom_metrics["disturbance_count"] = float(episode.user_data["disturbance"])
        episode.custom_metrics["total_disturbance_cost"] = float(episode.user_data["total_disturbance_cost"])
    ...
    def on_train_result(self, *, trainer, result: dict, **kwargs):
        ...
        if "disturbance_count_mean" in result["custom_metrics"]:
            result["disturbance"] = result["custom_metrics"]["disturbance_count_mean"]
...
class ILCallBack(HAIM_DRLCallbacks):
    def on_train_result(self, *, trainer, result: dict, **kwargs):
        result["success"] = np.nan
        result["crash"] = np.nan
        result["out"] = np.nan
        ...
        if "evaluation" in result:
            ...
            if "disturbance_count_mean" in eval["custom_metrics"]:
                result["disturbance"] = eval["custom_metrics"]["disturbance_count_mean"]