...
if __name__ == '__main__':
    ...
    while episode_num < num:
        ...
        if done:
            ...
            episode_disturbance_cost.append(total_disturbance_cost)
            total_reward = 0
            total_cost = 0
            total_disturbance_cost = 0
            ...
            print('reset:', episode_num, "this_episode_len:", last, "total_success_rate:", success / episode_num,
                  ...
                  "mean_episode_disturbance_cost:{}({})".format(np.mean(episode_disturbance_cost), np.std(episode_disturbance_cost))
                  )
            obs = env.reset()
            last = 0
            print('finish {}'.format(episode_num))

    data_set = {"data": pool, "episode_reward": episode_reward, "episode_cost": episode_cost, "episode_disturbance_cost": episode_disturbance_cost,
                "success_rate": success / episode_num, "episode_len": episode_len}
    try:
        with open('human_traj_' + str(num) + '_0.06.json', 'w') as f:
