...
def plot_result():
    """
    Plot: position, velocity, acceleration, spacing
    """
    fig = plt.figure()
    fig_x = fig.add_subplot(2, 2, 1)
    fig_v = fig.add_subplot(2, 2, 2)
    fig_a = fig.add_subplot(2, 2, 3)
    fig_d = fig.add_subplot(2, 2, 4)
    i = range(TIME)
    for n in range(NUM):
        fig_x.plot(i, x[n], label=f"Vehicle {n + 1}")
        fig_v.plot(i, v[n], label=f"Vehicle {n + 1}")
        fig_a.plot(i, a[n], label=f"Vehicle {n + 1}")
    for n in range(1, NUM):
        fig_d.plot(i, x[n-1]-x[n], label=f"Vehicle {n +1}-{n}")

    fig_x.set_xlabel('$time(s)$')
    fig_x.set_ylabel('$position(m)$')
    ...
    fig_d.set_xlabel('$time(s)$')
    fig_d.set_ylabel('$spacing(m)$')
    fig_d.legend()
