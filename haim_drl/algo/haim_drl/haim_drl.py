...
from haim_drl.algo.haim_drl.visual_model import VisualConstrainedSACModel
from haim_drl.algo.haim_drl.sac_lag_model import ConstrainedSACModel
from haim_drl.algo.sac_lag.sac_lag_policy import UpdatePenalty
from haim_drl.utils.replay_buffer import MyReplayBuffer
...
from haim_drl.algo.sac_lag.sac_lag import SACLagTrainer, validate_config
from haim_drl.algo.sac_lag.sac_lag_policy import SACPIDConfig, SACPIDPolicy, get_dist_class
...
# HAIM_DRL Romve the Lagrangian Multiplier from SACLag. Instead, it directly minimizes the takeover cost and disturbance cost
HAIM_DRLConfig = merge_dicts(SACPIDConfig,
                         {
                             "info_cost_key": "takeover_cost",
                             "info_total_cost_key": "total_takeover_cost",
                             "info_disturbance_cost_key": "disturbance_cost",
                             "info_total_disturbance_cost_key": "total_disturbance_cost",
                             # "twin_disturbance_cost_q" : False,
...
def validate_saver_config(config):
    ...
    assert config["info_disturbance_cost_key"] == "disturbance_cost" and config["info_total_disturbance_cost_key"] == "total_disturbance_cost"
...
def postprocess_trajectory(policy,
    ...
    if (infos is not None) and (infos[0] != 0.0):
        ...
        sample_batch[policy.config["info_disturbance_cost_key"]] = np.array(
            [info[policy.config["info_disturbance_cost_key"]] for info in sample_batch[SampleBatch.INFOS]]
        ).astype(sample_batch[SampleBatch.REWARDS].dtype)
        sample_batch[policy.config["info_total_disturbance_cost_key"]] = np.array(
            [info[policy.config["info_total_disturbance_cost_key"]] for info in sample_batch[SampleBatch.INFOS]]
        ).astype(sample_batch[SampleBatch.REWARDS].dtype)
        ...
        sample_batch[policy.config["info_disturbance_cost_key"]] = np.zeros_like(sample_batch[SampleBatch.REWARDS])
        sample_batch[policy.config["info_total_disturbance_cost_key"]] = np.zeros_like(sample_batch[SampleBatch.REWARDS])
    ...
    assert policy.config["info_disturbance_cost_key"] in batch
    assert policy.config["info_total_disturbance_cost_key"] in batch
...
def sac_actor_critic_loss(policy, model, _, train_batch):
    ...
    _ = train_batch[policy.config["info_total_disturbance_cost_key"]]  # Touch this item, this is helpful in ray 1.2.0
    ...
    # Disturbance cost Q-Value for actually selected actions
    disturbance_c_q_t = model.get_disturbance_cost_q_values(model_out_t,
                                    train_batch[NEWBIE_ACTION])
    if policy.config["twin_cost_q"]:
        twin_disturbance_c_q_t = model.get_twin_disturbance_cost_q_values(
            model_out_t,
            train_batch[NEWBIE_ACTION])
    ...
    # Disturbance cost Q-values for current policy in given current state.
    disturbance_c_q_t_det_policy = model.get_disturbance_cost_q_values(model_out_t, policy_t)
    if policy.config["twin_cost_q"]:
        twin_disturbance_c_q_t_det_policy = model.get_twin_disturbance_cost_q_values(
            model_out_t, policy_t)
        disturbance_c_q_t_det_policy = tf.reduce_min(
            (disturbance_c_q_t_det_policy, twin_disturbance_c_q_t_det_policy), axis=0)
    ...
    # Disturbance target c-q network evaluation
    disturbance_c_q_tp1 = policy.target_model.get_disturbance_cost_q_values(target_model_out_tp1,
                                                    policy_tp1)
    if policy.config["twin_cost_q"]:
        twin_disturbance_c_q_tp1 = policy.target_model.get_twin_disturbance_cost_q_values(
            target_model_out_tp1, policy_tp1)
        # Take min over both twin-NNs.
        c_q_tp1 = tf.reduce_min((disturbance_c_q_tp1, twin_disturbance_c_q_tp1), axis=0)
    ...
    # Disturbance c_q_t selected
    disturbance_c_q_t_selected = tf.squeeze(c_q_t, axis=len(disturbance_c_q_t.shape) - 1)
    if policy.config["twin_cost_q"]:
        twin_disturbance_c_q_t_selected = tf.squeeze(twin_disturbance_c_q_t, axis=len(twin_c_q_t.shape) - 1)

    # Target Q plus entropy
    ...
    disturbance_c_q_tp1_best = tf.squeeze(input=disturbance_c_q_tp1, axis=len(disturbance_c_q_tp1.shape) - 1)
    disturbance_c_q_tp1_best_masked = \
        (1.0 - tf.cast(train_batch[SampleBatch.DONES], tf.float32)) * \
        disturbance_c_q_tp1_best
    ...
    # Compute disturbance Cost of bellman equation.
    disturbance_c_q_t_selected_target = tf.stop_gradient(train_batch[policy.config["info_disturbance_cost_key"]] +
                                             policy.config["gamma"] ** policy.config["n_step"] * disturbance_c_q_tp1_best_masked)
    ...
    # Compute the disturbance Cost TD-error (potentially clipped).
    base_disturbance_c_td_error = tf.math.abs(disturbance_c_q_t_selected - disturbance_c_q_t_selected_target)
    if policy.config["twin_cost_q"]:
        twin_disturbance_c_td_error = tf.math.abs(twin_disturbance_c_q_t_selected - disturbance_c_q_t_selected_target)
        disturbance_c_td_error = 0.5 * (base_disturbance_c_td_error + twin_disturbance_c_td_error)
    else:
        disturbance_c_td_error = base_disturbance_c_td_error
    ...
    # add disturbance cost critic
    critic_loss.append(
        0.5 * tf.keras.losses.MSE(
            y_true=disturbance_c_q_t_selected_target, y_pred=disturbance_c_q_t_selected))
    if policy.config["twin_cost_q"]:
        critic_loss.append(0.5 * tf.keras.losses.MSE(
            y_true=disturbance_c_q_t_selected_target, y_pred=twin_disturbance_c_q_t_selected))
    ...
    if model.discrete:
        ...
        disturbance_cost_loss = tf.reduce_mean(disturbance_c_q_t_det_policy)
        # actor_loss = tf.reduce_mean(
        #     model.alpha * log_pis_t - q_t_det_policy + c_q_t_det_policy)
        actor_loss = tf.reduce_mean(
            model.alpha * log_pis_t - q_t_det_policy + c_q_t_det_policy + disturbance_c_q_t_det_policy)

    # save for stats function
    policy.policy_t = policy_t
    policy.cost_loss = cost_loss
    policy.disturbance_cost_loss = disturbance_cost_loss
    policy.reward_loss = reward_loss
    policy.mean_batch_cost = train_batch[policy.config["info_cost_key"]]
    policy.mean_batch_disturbance_cost = train_batch[policy.config["info_disturbance_cost_key"]]
    policy.q_t = q_t
    policy.c_q_tp1 = c_q_tp1
    policy.c_q_t = c_q_t
    policy.disturbance_c_q_tp1 = disturbance_c_q_tp1
    policy.disturbance_c_q_t = disturbance_c_q_t
    policy.td_error = td_error
    policy.c_td_error = c_td_error
    policy.disturbance_c_td_error = disturbance_c_td_error
    policy.actor_loss = actor_loss
    policy.critic_loss = critic_loss
    policy.c_td_target = c_q_t_selected_target
    policy.disturbance_c_td_target = disturbance_c_q_t_selected_target
...
def build_sac_model(policy, obs_space, action_space, config):
    ...
    policy.model = ModelCatalog.get_model_v2(
        ...
        # twin_disturbance_cost_q=config["twin_disturbance_cost_q"],
...
HAIM_DRLPolicy = SACPIDPolicy.with_updates(name="HAIM_DRL",
                                       make_model=build_sac_model,
                                       get_default_config=lambda: HAIM_DRLConfig,
                                       postprocess_fn=postprocess_trajectory,
                                       loss_fn=sac_actor_critic_loss)

HAIM_DRLTrainer = SACLagTrainer.with_updates(name="HAIM_DRL",
                                         default_config=HAIM_DRLConfig,
                                         default_policy=HAIM_DRLPolicy,
                                         get_policy_class=lambda config: HAIM_DRLPolicy,
