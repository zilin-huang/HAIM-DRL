...
class VisualConstrainedSACModel(TFModelV2):
    ...
    def __init__(
        ...
        self.disturbance_cost_q_net = build_q_net(
            "disturbance_cost_q", self.model_out, self.actions_input
        )
        self.register_variables(self.q_net.variables)
        self.register_variables(self.cost_q_net.variables)
        self.register_variables(self.disturbance_cost_q_net.variables)
        ...
        if twin_cost_q:
            ...
            self.disturbance_cost_twin_q_net = build_q_net(
                "disturbance_cost_twin_q", self.model_out, self.actions_input
            )
            self.register_variables(self.disturbance_cost_twin_q_net.variables)
        else:
            self.cost_twin_q_net = None
            self.disturbance_cost_twin_q_net = None

        #
        # if twin_disturbance_cost_q:
        #     self.disturbance_cost_twin_q_net = build_q_net(
        #         "disturbance_cost_twin_q", self.model_out, self.actions_input
        #     )
        #     self.register_variables(self.disturbance_cost_twin_q_net.variables)
    ...
    def get_disturbance_cost_q_values(self, model_out, actions=None):
        model_out = self.get_model_out(model_out)
        if actions is not None:
            return self.disturbance_cost_q_net([model_out, actions])
        else:
            return self.disturbance_cost_q_net(model_out)

    def get_twin_disturbance_cost_q_values(self, model_out, actions=None):
        model_out = self.get_model_out(model_out)
        if actions is not None:
            return self.disturbance_cost_twin_q_net([model_out, actions])
        else:
            return self.disturbance_cost_twin_q_net(model_out)
    ...
    def disturbance_cost_q_variables(self):
        return self.disturbance_cost_q_net.variables + self.conv_model.variables + (
            self.disturbance_cost_twin_q_net.variables
            if self.disturbance_cost_twin_q_net else []
        )
